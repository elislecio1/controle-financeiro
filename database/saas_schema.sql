-- Schema para Sistema SaaS Multi-Tenant
...
CREATE TABLE IF NOT EXISTS tenants (
    ...
    status TEXT DEFAULT 'pending' CHECK (status IN ('active', 'suspended', 'cancelled', 'pending')),
...
-- Políticas RLS para subscription_plans (público)
...
CREATE POLICY "Subscriptions are viewable by tenant admins" ON subscriptions
    FOR SELECT USING (
        auth.uid() IN (
            SELECT user_id FROM user_profiles 
            WHERE tenant_id = subscriptions.tenant_id AND role = 'admin'
        )
    );

-- Políticas RLS para billing_info
CREATE POLICY "Billing info is viewable by tenant admins" ON billing_info
    FOR SELECT USING (
        auth.uid() IN (
            SELECT user_id FROM user_profiles 
            WHERE tenant_id = billing_info.tenant_id AND role = 'admin'
        )
    );

CREATE POLICY "Billing info can be updated by tenant admins" ON billing_info
    FOR UPDATE USING (
        auth.uid() IN (
            SELECT user_id FROM user_profiles 
            WHERE tenant_id = billing_info.tenant_id AND role = 'admin'
...
CREATE POLICY "API keys are viewable by tenant admins" ON api_keys
    FOR SELECT USING (
        auth.uid() IN (
            SELECT user_id FROM user_profiles 
            WHERE tenant_id = api_keys.tenant_id AND role = 'admin'
        )
    );

CREATE POLICY "API keys can be managed by tenant admins" ON api_keys
    FOR ALL USING (
        auth.uid() IN (
            SELECT user_id FROM user_profiles 
            WHERE tenant_id = api_keys.tenant_id AND role = 'admin'
...
CREATE POLICY "Webhooks are viewable by tenant admins" ON webhooks
    FOR SELECT USING (
        auth.uid() IN (
            SELECT user_id FROM user_profiles 
            WHERE tenant_id = webhooks.tenant_id AND role = 'admin'
        )
    );

CREATE POLICY "Webhooks can be managed by tenant admins" ON webhooks
    FOR ALL USING (
        auth.uid() IN (
            SELECT user_id FROM user_profiles 
            WHERE tenant_id = webhooks.tenant_id AND role = 'admin'
...
CREATE POLICY "Activity logs are viewable by tenant admins" ON activity_logs
    FOR SELECT USING (
        auth.uid() IN (
            SELECT user_id FROM user_profiles 
            WHERE tenant_id = activity_logs.tenant_id AND role = 'admin'
        )
    );
...
ALTER TABLE subcategorias ADD COLUMN IF NOT EXISTS tenant_id UUID REFERENCES tenants(id);
...
ALTER TABLE orcamentos ADD COLUMN IF NOT EXISTS tenant_id UUID REFERENCES tenants(id);
ALTER TABLE investimentos ADD COLUMN IF NOT EXISTS tenant_id UUID REFERENCES tenants(id);
ALTER TABLE contas_bancarias ADD COLUMN IF NOT EXISTS tenant_id UUID REFERENCES tenants(id);
ALTER TABLE cartoes_credito ADD COLUMN IF NOT EXISTS tenant_id UUID REFERENCES tenants(id);
ALTER TABLE contatos ADD COLUMN IF NOT EXISTS tenant_id UUID REFERENCES tenants(id);

-- Índices para tenant_id nas tabelas existentes
CREATE INDEX IF NOT EXISTS idx_user_profiles_tenant_id ON user_profiles(tenant_id);
CREATE INDEX IF NOT EXISTS idx_transactions_tenant_id ON transactions(tenant_id);
CREATE INDEX IF NOT EXISTS idx_categorias_tenant_id ON categorias(tenant_id);
CREATE INDEX IF NOT EXISTS idx_subcategorias_tenant_id ON subcategorias(tenant_id);
CREATE INDEX IF NOT EXISTS idx_centros_custo_tenant_id ON centros_custo(tenant_id);
CREATE INDEX IF NOT EXISTS idx_metas_tenant_id ON metas(tenant_id);
...
CREATE INDEX IF NOT EXISTS idx_cartoes_credito_tenant_id ON cartoes_credito(tenant_id);
CREATE INDEX IF NOT EXISTS idx_contatos_tenant_id ON contatos(tenant_id);

-- Atualizar políticas RLS existentes para incluir tenant_id
DROP POLICY IF EXISTS "Users can view their own profile" ON user_profiles;
CREATE POLICY "Users can view their own profile" ON user_profiles
    FOR SELECT USING (
        auth.uid() = user_id OR 
        (tenant_id IS NOT NULL AND auth.uid() IN (
            SELECT user_id FROM user_profiles 
            WHERE tenant_id = user_profiles.tenant_id AND role = 'admin'
        ))
    );
