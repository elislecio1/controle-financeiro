-- Script para criar as tabelas de integrações bancárias
...
CREATE INDEX IF NOT EXISTS idx_transacoes_importadas_status ON transacoes_importadas(status_conciliação);
...
CREATE POLICY "Allow all operations for authenticated users" ON integracoes_bancarias
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow all operations for authenticated users" ON logs_sincronizacao
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow all operations for authenticated users" ON transacoes_importadas
    FOR ALL USING (auth.role() = 'authenticated');
