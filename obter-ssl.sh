...
echo "🔒 Obtendo certificado SSL para financeiro.donsantosba.com.br"
echo ""
...
WEBROOT="/www/wwwroot/financeiro.donsantosba.com.br"

# Verificar se é root
if [ "$EUID" -ne 0 ]; then 
    echo -e "${RED}❌ Por favor, execute com sudo${NC}"
    echo "Uso: sudo ./obter-ssl.sh"
    exit 1
fi
...
if [ "$METHOD" == "webroot" ]; then
    certbot certonly --webroot -w $WEBROOT -d $DOMAIN --email admin@donsantosba.com.br --agree-tos --non-interactive
else
    echo -e "${YELLOW}⚠ Parando Nginx temporariamente...${NC}"
    systemctl stop nginx 2>/dev/null || service nginx stop 2>/dev/null
    
    certbot certonly --standalone -d $DOMAIN --email admin@donsantosba.com.br --agree-tos --non-interactive
...
if [ $? -eq 0 ]; then
    ...
    echo "1. Configure o Nginx com SSL (use nginx-financeiro.conf)"
